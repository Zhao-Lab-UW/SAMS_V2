...
%   Updated versions of input variables
    ...
    if ~isempty(new_idx_list)
        ...
        if ~isempty(pptx)
            ...
            raster_raw{num_electrode, 2, num_well} = [i, j, m, n, 1];
        ...
        % Update electrode statistics table
        Tleft_electrode = table([size(current_spikes, 2); length(unsorted_WFs); ...
            size(current_spikes, 2) / maxTime; length(new_idx_list)]);
